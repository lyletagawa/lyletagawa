...
Standard CDN edge caching works well for video-on-demand. Popular films accumulate requests. Niche titles gradually drop out of cache. LRU aligns cache incentives with actual demand (Hasslinger et al., 2023).
...
Age is derived from the last-modified header the origin server returns when delivering the segment to the CDN (Newton, 2026).
