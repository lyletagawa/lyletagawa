...
**Temperature shapes answers, not just randomness.** After the final layer, each token's output vector multiplies against the embedding matrix to produce one score per vocabulary entry. Temperature scales those scores before softmax. Low temperature makes the model pick the highest-probability token almost every time. High temperature flattens the distribution and pulls in lower-probability options. Top-k sampling is an alternative which restricts the pool to the k highest-scoring candidates before sampling {{< cite 9 "Alammar, Jay (2019). The Illustrated GPT-2. jalammar.github.io." >}}. The same prompt with different sampling settings can produce radically different outputs.
